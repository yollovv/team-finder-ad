...
Django backend для платформы поиска команды в pet-проекты.
Реализованы:
...
- участие в проектах;
- избранные проекты;
- фильтры участников для варианта 1.
...
Используйте те же параметры в `.env`.
...
После заполнения `.env` и настройки базы данных выполните миграции:

```bash
python manage.py migrate
```

Опционально: создайте демо-данные (несколько пользователей и проекты):

```bash
python manage.py seed_demo_data
```

После этого можно запускать сервер разработки:
